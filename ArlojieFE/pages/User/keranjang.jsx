...
useEffect(() => {
  ...
  const fetchCartItems = async () => {
    ...
    try {
      ...
      const sanitizedItems = itemsFromApi.map((item) => {
        let images = [];
        try {
    ...
    // FUNGSI SETELAH SUBMIT ALAMAT (SUDAH BENAR)
    const handleAlamatSubmit = (dataAlamat) => {
        setShowAlamatForm(false);
        navigate("/checkout", { 
            state: { alamat: dataAlamat, cartItems: cartItems } 
        });
    };
    
    // KALKULASI TOTAL (SUDAH BENAR)
    const subtotal = cartItems.reduce(
        (sum, item) => sum + (item.product?.price || 0) * item.quantity, 0
    );
    const grandTotal = subtotal > 0 ? subtotal + estimatedShipping : 0;

    // TAMPILAN LOADING
    if (loading) {
        return <div className="min-h-screen flex justify-center items-center">Memuat Keranjang...</div>;
    }
  return (
